...
function ReviewsPage({ apiRequest, decisions, openDecision, currentUser, globalSearch = "", onApprovalChange }) {
  ...
  const act = async (approvalId, status) => {
    try {
      ...
      await apiRequest(`/api/approvals/${approvalId}`, { method: "PATCH", body: JSON.stringify({ status, comments: comments[approvalId]?.trim() || null }) });
      await load();
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((approval) => {
      ...
      return [decision?.title, approval.requestedBy?.name, approval.level, approval.status].filter(Boolean).join(" ").toLowerCase().includes(query);
    });
  ...
  return (
    <section className="workspace-page full-module-page review-workspace-page">
      <div className="workspace-page-header">
        <div><span className="section-label">APPROVAL WORKFLOW</span><h1>Reviews</h1><p>{currentUser?.role === "Manager" ? "Complete final approvals after reviewer sign-off." : "Review decisions assigned to you and keep approvals moving."}</p></div>
        <button className="compact-action" type="button" onClick={load}>Refresh</button>
      </div>
      <div className="module-stat-grid">
        <div className="module-stat-card"><span>Pending reviews</span><strong>{filteredItems.length}</strong><small>Assigned to you</small></div>
        <div className="module-stat-card"><span>Stage</span><strong>{currentUser?.role === "Manager" ? "Final" : "Review"}</strong><small>{currentUser?.role || "Reviewer"} workspace</small></div>
      </div>
      <div className="review-module-list">
        {loading ? <EmptyState icon={Clock3} title="Loading reviews" body="Fetching approval requests from the workspace." /> : filteredItems.length === 0 ? <EmptyState icon={CheckCircle2} title={globalSearch ? "No reviews found" : "You’re all caught up"} body={globalSearch ? "Try a different search term." : "There are no pending decisions waiting for you."} /> : filteredItems.map((approval) => {
          const decision = decisions.find((item) => item.id === approval.decisionId) || approval.decision;
          return <article className="review-module-card" key={approval.id}>
...
function TeamsPage({ apiRequest, currentUser, globalSearch = "" }) {
  ...
  const canCreateTeam = ["Manager", "Administrator"].includes(currentUser?.role);
  const canManageMembers = currentUser?.role === "Administrator" || ["Owner", "Manager", "Lead"].includes(myMembership?.teamRole);
  ...
  return (
    <section className="workspace-page full-module-page">
      <div className="workspace-page-header">
        ...
        {canCreateTeam && <button className="new-decision-button" type="button" onClick={() => setShowCreate((value) => !value)}><span>+</span>New team</button>}
      </div>
...
function KnowledgePage({ apiRequest, globalSearch = "", openDecision }) {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError("");
      const data = await apiRequest("/api/knowledge?tab=decisions&page=1&pageSize=50");
      setDecisions(data?.decisions || []);
    } catch (requestError) {
      setError(requestError.message || "Unable to load the knowledge repository.");
      setDecisions([]);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return decisions.filter((decision) => {
      const matchesQuery = !query || [decision.title, decision.problemStatement, decision.team?.name, decision.createdBy?.name, ...(decision.documents || []).map((doc) => doc.filename)].join(" ").toLowerCase().includes(query);
      const matchesStatus = statusFilter === "all" || decision.status === statusFilter;
      return matchesQuery && matchesStatus;
    });
  }, [decisions, search, statusFilter]);

  const totalDocuments = decisions.reduce((sum, decision) => sum + (decision.documents?.length || 0), 0);
  ...
  return (
    <section className="workspace-page full-module-page knowledge-redesign-page">
      <div className="workspace-page-header knowledge-header">
        <div><span className="section-label">KNOWLEDGE</span><h1>Knowledge Repository</h1><p>Decision-centered knowledge: every decision keeps its supporting evidence, context and history together.</p></div>
        <button className="compact-action" type="button" onClick={load}>Refresh</button>
      </div>
      ...
      <div className="knowledge-overview-strip">
        ...
        <div><span>Evidence coverage</span><strong>{decisions.length ? Math.round((decisions.filter((decision) => (decision.documents || []).length > 0).length / decisions.length) * 100) : 0}%</strong></div>
      </div>

      <div className="knowledge-controls">
        <div className="dashboard-search inline-search"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search decisions, documents, teams or people..." /></div>
        <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">All statuses</option><option value="Draft">Draft</option><option value="UnderReview">Under review</option><option value="Approved">Approved</option><option value="Rejected">Rejected</option><option value="Archived">Archived</option></select>
        <span>{filtered.length} decision{filtered.length === 1 ? "" : "s"}</span>
      </div>

      <div className="knowledge-decision-grid">
        {loading ? <EmptyState icon={Clock3} title="Loading knowledge" body="Fetching decisions and their supporting documents." /> : error ? <div className="module-error-state"><X size={18} /><strong>Knowledge unavailable</strong><span>{error}</span><button type="button" onClick={load}>Try again</button></div> : filtered.length ? filtered.map((decision) => {
          const isOpen = expanded === decision.id;
          const docs = decision.documents || [];
          return <article className={`knowledge-decision-card ${isOpen ? "expanded" : ""}`} key={decision.id}>
            <button className="knowledge-decision-card-head" type="button" onClick={() => setExpanded((current) => current === decision.id ? null : decision.id)}>
              <div className="knowledge-decision-icon"><CheckCircle2 size={18} /></div>
              <div className="knowledge-decision-head-copy"><div className="knowledge-card-kicker">DECISION KNOWLEDGE</div><strong>{decision.title}</strong><span>{decision.problemStatement || "No problem statement added."}</span><small>{decision.team?.name || "Unassigned"} · {decision.createdBy?.name || "Workspace member"} · Updated {formatDate(decision.updatedAt)}</small></div>
              <div className="knowledge-decision-head-meta"><span className={`status status-${statusClass(decision.status)}`}><span />{statusLabel(decision.status)}</span><span className="knowledge-document-count">{docs.length} document{docs.length === 1 ? "" : "s"}</span><ChevronRight className={isOpen ? "knowledge-chevron open" : "knowledge-chevron"} size={18} /></div>
            </button>

            {isOpen && <div className="knowledge-decision-expanded">
              <div className="knowledge-expanded-toolbar"><div><strong>Supporting documents</strong><span>{docs.length ? "Evidence attached to this decision" : "No documents attached yet"}</span></div>{openDecision && <button type="button" className="knowledge-open-decision" onClick={() => openDecision(decision)}>Open full decision <ArrowUpRight size={13} /></button>}</div>
              {docs.length ? <div className="knowledge-document-stack">{docs.map((doc) => <div className="knowledge-document-item" key={doc.id}>
                <div className="knowledge-file-icon">{String(doc.filename).toLowerCase().endsWith(".pdf") ? "PDF" : "FILE"}</div>
                <div className="knowledge-document-item-copy"><strong>{doc.filename}</strong><span>{doc.category || "General"} · Uploaded by {doc.uploadedBy?.name || "Workspace member"} · {formatDate(doc.createdAt)}</span>{doc.tags ? <div className="tag-list">{doc.tags.split(",").map((tag) => tag.trim()).filter(Boolean).slice(0, 4).map((tag) => <span key={tag}>{tag}</span>)}</div> : null}</div>
                <a className="knowledge-view-button" href={`${API_BASE_URL}/${String(doc.filePath || "").replaceAll("\\", "/")}`} target="_blank" rel="noreferrer">Open <ArrowUpRight size={13} /></a>
              </div>)}</div> : <EmptyState icon={FolderOpen} title="No supporting documents" body="Upload evidence from the decision workspace to build this knowledge record." />}
            </div>}
          </article>;
        }) : <EmptyState icon={Search} title="No matching decisions" body="Try a different search or status filter." />}
      </div>
    </section>
  );
}
...
function ReportsPage({ apiRequest, globalSearch = "" }) {
  ...
  const query = globalSearch.trim().toLowerCase();
  const visibleReportTypes = reportTypes.filter(([, title, description]) =>
  ...
  return <section className="workspace-page full-module-page"><div className="workspace-page-header"><div><span className="section-label">REPORTING</span><h1>Reports</h1><p>Generate structured exports for decisions, approvals, teams and governance.</p></div></div><div className="report-card-grid">{visibleReportTypes.length ? visibleReportTypes.map(([key, title, description]) => <article className="report-card" key={key}><div className="report-card-icon"><FileText size={19} /></div><div><span className="panel-kicker">EXPORT</span><h3>{title}</h3><p>{description}</p></div><div className="report-card-actions"><button className="modal-primary" type="button" onClick={() => download(key)}>Excel / CSV <ArrowUpRight size={13} /></button><button className="modal-secondary" type="button" onClick={() => print(key)}>Print / PDF <ArrowUpRight size={13} /></button></div></article>) : <EmptyState icon={FileText} title="No reports found" body="Try a different search term." />}</div></section>;
}
