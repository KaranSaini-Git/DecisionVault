...
function ApprovalPanel({
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await apiRequest(
        `/api/decisions/${selectedDecision.id}/approvals`,
      );
  ...
  const myPendingApproval = approvals.find(
    (approval) =>
      approval.status === "Pending" && approval.reviewerId === currentUser?.id,
  );

  const canRequest = ["Manager", "Administrator"].includes(currentUser?.role);
  const canAct =
  ...
  return (
    <div className="approval-panel">
      <div className="approval-status-banner">
        <div className="approval-panel-icon">
          <ShieldCheck size={19} />
        </div>
        <div>
          <strong>
            {selectedDecision?.status === "UnderReview"
              ? "Approval in progress"
              : selectedDecision?.status}
          </strong>
          <span>
            {selectedDecision?.status === "UnderReview"
              ? "This decision is waiting for one or more assigned reviewers."
              : "Approval history and reviewer activity for this decision."}
          </span>
        </div>
      </div>
      ...
      {myPendingApproval && canAct && (
        <div className="approval-review-panel">
          <div>
            <div className="panel-kicker">YOUR REVIEW</div>
            <h3>Decision assigned to you</h3>
            <p>Complete this approval directly from the decision workspace.</p>
          </div>
          <textarea
            rows="3"
            value={comment}
      ...
      {canRequest && selectedDecision?.status !== "Approved" && (
        <form className="approval-request-panel" onSubmit={request}>
          <div>
            <div className="panel-kicker">REQUEST APPROVAL</div>
            <h3>Send this decision for review</h3>
            <p>Assign a reviewer and approval level.</p>
          </div>
          <div className="approval-request-grid">
            ...
            >
              <option value={1}>Level 1 · Reviewer</option>
      ...
      <div className="approval-history-list">
        {loading ? (
          <div className="module-inline-empty">
            <Clock3 size={17} />
          ...
          <div className="module-inline-empty">
            <ShieldCheck size={17} />
            <span>No approval requests have been created yet.</span>
          </div>
        ) : (
          approvals.map((approval) => (
            <article className="approval-history-card" key={approval.id}>
              <div
                className={`approval-history-icon ${statusClass(approval.status)}`}
              >
              ...
              <div className="approval-history-copy">
                <div>
                  <strong>
                    Level {approval.level} ·{" "}
                    {approval.reviewer?.name || "Assigned reviewer"}
                  </strong>
                  <span
                    className={`approval-badge ${statusClass(approval.status)}`}
                  >
                    {approval.status}
                  </span>
                </div>
      ...
      {latest?.status === "Approved" && (
        <div className="approval-complete-note">
          <CheckCircle2 size={17} /> Approval workflow completed.
        </div>
      )}
    </div>
  );
