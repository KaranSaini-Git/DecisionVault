  ...
  try {
    if (!["Manager", "Administrator"].includes(req.user.role)) {
      return res.status(403).json({
        message:
          "Only managers or administrators can manually request approvals",
      });
    }
    ...
    if (![1, 2].includes(level))
      return res.status(400).json({ message: "Approval level must be 1 or 2" });
    ...
    if (!(await canManageDecision(decision, req.user))) {
      return res.status(403).json({
        message:
          "You do not have permission to request approval for this decision",
      });
    }
    ...
    if (reviewer.role !== requiredRole)
      return res.status(400).json({
        message: `Level ${level} approvals require a ${requiredRole}`,
      });

    if (decision.teamId) {
      const reviewerMembership = await prisma.teamMember.findUnique({
        where: {
          teamId_userId: { teamId: decision.teamId, userId: reviewerId },
        },
      });
      if (!reviewerMembership) {
        return res.status(400).json({
          message: "The assigned approver must belong to the decision's team",
        });
      }
    }
    ...
    if (level === 2) {
      ...
      if (
        !levelOneApprovals.length ||
        levelOneApprovals.some((item) => item.status !== "Approved")
      ) {
        return res.status(409).json({
          message:
            "Level 1 reviewer approval must be completed before requesting Level 2 approval",
        });
      }
    }

    const existingApproval = await prisma.approval.findUnique({
      where: { decisionId_reviewerId_level: { decisionId, reviewerId, level } },
    });
    if (existingApproval)
      return res
        .status(409)
        .json({ message: "Approval request already exists" });
    ...
    await createNotification({
      ...
      message: `You have a Level ${level} approval request for ${decision.title}.`,
      entityType: "Decision",
      entityId: decisionId,
    });
    await logActivity({
      ...
      metadata: { reviewerId, level, automatic: false },
    });

    res
      .status(201)
      .json({ message: "Approval requested successfully", approval });
  } catch (error) {
...
const findNextManager = async (decision, excludeId) => {
  const where = {
    ...
    id: { not: excludeId },
    ...(decision.teamId
      ? { teamMemberships: { some: { teamId: decision.teamId } } }
      : {}),
  };
  return (
    ...
    prisma.user.findFirst({
      where: { role: "Manager", id: { not: excludeId } },
      select: { id: true, name: true, email: true, role: true },
      orderBy: { id: "asc" },
    })
  );
...
const actOnApproval = async (req, res) => {
  try {
    ...
    const approval = await prisma.approval.findUnique({
      where: { id: approvalId },
      include: {
        decision: { include: { team: { select: { id: true, name: true } } } },
    ...
    if (approval.reviewerId !== userId)
      return res
        .status(403)
        .json({ message: "You are not assigned to this approval" });
    if (approval.status !== "Pending")
      return res
        .status(400)
        .json({ message: "This approval has already been processed" });

    const requiredRole = approval.level === 1 ? "Reviewer" : "Manager";
    if (req.user.role !== requiredRole)
      return res.status(403).json({
        message: `${requiredRole}s are the only users who can complete Level ${approval.level} approvals`,
      });
    ...
    const updatedApproval = await prisma.$transaction(async (tx) => {
      if (status === "Approved" && approval.level > 1) {
        const levelOne = await tx.approval.findMany({
          where: { decisionId: approval.decisionId, level: 1 },
        });
        if (
          !levelOne.length ||
          levelOne.some((item) => item.status !== "Approved")
        ) {
      ...
      if (approval.level === 1) {
        const managerWhere = {
          role: "Manager",
          id: { not: approval.decision.createdById },
          ...(approval.decision.teamId
            ? {
                teamMemberships: { some: { teamId: approval.decision.teamId } },
              }
            : {}),
        };
    ...
    await createNotification({
      ...
      message: `${approval.decision.title} was ${status.toLowerCase()} by ${approval.reviewer?.name || "the assigned approver"}.`,
      entityType: "Decision",
    ...
    if (
      ...
      await createNotification({
        ...
        message: `${approval.decision.title} passed reviewer approval and is now waiting for your final approval.`,
        entityType: "Decision",
        entityId: approval.decisionId,
      });
      await logActivity({
        ...
        decisionId: approval.decisionId,
        teamId: approval.decision.teamId,
        metadata: { fromLevel: 1, toLevel: 2, managerId: nextManager.id },
      });
    }
    ...
    await logActivity({
      ...
      decisionId: approval.decisionId,
      teamId: approval.decision.teamId,
      metadata: { level: approval.level, comments: comments || "" },
    });
    ...
    if (error?.message === "EARLIER_STAGE_NOT_COMPLETE")
      return res
        .status(409)
        .json({ message: "Earlier approval stages must be completed first" });
    if (error?.message === "NO_MANAGER_AVAILABLE")
      return res.status(409).json({
        message: "No Manager is available to receive the final approval",
      });
...
const getMyPendingApprovals = async (req, res) => {
  try {
    ...
    const allowed = approvals.filter(
      (approval) =>
        (approval.level === 1 && req.user.role === "Reviewer") ||
        (approval.level === 2 && req.user.role === "Manager"),
    );
