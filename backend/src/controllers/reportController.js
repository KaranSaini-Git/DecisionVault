...
const buildReportRows = async (type, user) => {
  ...
  if (type === "decisions") {
    ...
    return {
      ...
      rows: rows.map((row) => [
        ...
        row.createdBy?.name,
        row.team?.name,
        row._count.alternatives,
        row._count.documents,
        row._count.discussions,
        row._count.approvals,
  ...
  const rows = await prisma.auditLog.findMany({
    where:
      user?.role === "Manager"
        ? { OR: [{ userId: user.userId }, { decision: decisionWhere }] }
        : {},
...
const getReport = async (req, res) => {
  try {
    ...
    const type = ["decisions", "approvals", "teams", "audit"].includes(
      req.params.type,
    )
      ? req.params.type
      : "decisions";
